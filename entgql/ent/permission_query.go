...
func (_q *PermissionQuery) QueryAdminCreatedBy() *AdminUserQuery {
...
func (_q *PermissionQuery) QueryAdminUpdatedBy() *AdminUserQuery {
	query := (&AdminUserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *PermissionQuery) WithAdminCreatedBy(opts ...func(*AdminUserQuery)) *PermissionQuery {
	query := (&AdminUserClient{config: _q.config}).Query()
...
func (_q *PermissionQuery) WithAdminUpdatedBy(opts ...func(*AdminUserQuery)) *PermissionQuery {
	query := (&AdminUserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Permission, error) {
	...
	if _q.withAdminCreatedBy != nil || _q.withAdminUpdatedBy != nil || _q.withRole != nil {
		withFKs = true
	}
	if withFKs {
...
func (_q *PermissionQuery) loadAdminCreatedBy(ctx context.Context, query *AdminUserQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *AdminUser)) error {
...
func (_q *PermissionQuery) loadAdminUpdatedBy(ctx context.Context, query *AdminUserQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *AdminUser)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Permission)
	for i := range nodes {
...
func (_q *PermissionQuery) loadRole(ctx context.Context, query *RoleQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *Role)) error {
	...
	nodeids := make(map[string][]*Permission)
	for i := range nodes {
		if nodes[i].permission_role == nil {
			continue
		}
		fk := *nodes[i].permission_role
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "permission_role" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
