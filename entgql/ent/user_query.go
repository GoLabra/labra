...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withRefCreatedBy; query != nil {
		if err := uq.loadRefCreatedBy(ctx, query, nodes,
	...
	if query := uq.withRefUpdatedBy; query != nil {
		if err := uq.loadRefUpdatedBy(ctx, query, nodes,
	...
	if query := uq.withDefaultRole; query != nil {
		if err := uq.loadDefaultRole(ctx, query, nodes, nil,
			func(n *User, e *Role) { n.Edges.DefaultRole = e }); err != nil {
			return nil, err
		}
	}
	for name, query := range uq.withNamedRefCreatedBy {
		if err := uq.loadRefCreatedBy(ctx, query, nodes,
	...
	for name, query := range uq.withNamedRefUpdatedBy {
		if err := uq.loadRefUpdatedBy(ctx, query, nodes,
...
func (uq *UserQuery) loadRefCreatedBy(ctx context.Context, query *UserQuery, nodes []*User, init func(*User), assign func(*User, *User)) error {
...
func (uq *UserQuery) loadRefUpdatedBy(ctx context.Context, query *UserQuery, nodes []*User, init func(*User), assign func(*User, *User)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (uq *UserQuery) loadDefaultRole(ctx context.Context, query *RoleQuery, nodes []*User, init func(*User), assign func(*User, *Role)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*User)
	for i := range nodes {
...
func (uq *UserQuery) WithNamedRefCreatedBy(name string, opts ...func(*UserQuery)) *UserQuery {
	...
	if uq.withNamedRefCreatedBy == nil {
		uq.withNamedRefCreatedBy = make(map[string]*UserQuery)
	}
	uq.withNamedRefCreatedBy[name] = query
	return uq
}
...
func (uq *UserQuery) WithNamedRefUpdatedBy(name string, opts ...func(*UserQuery)) *UserQuery {
	...
	if uq.withNamedRefUpdatedBy == nil {
		uq.withNamedRefUpdatedBy = make(map[string]*UserQuery)
	}
	uq.withNamedRefUpdatedBy[name] = query
	return uq
}
