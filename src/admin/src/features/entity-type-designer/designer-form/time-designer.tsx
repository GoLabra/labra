...
export const schema = z.object({
    ...
    min: z.literal("").transform(() => undefined).or(z.any().transform((val) => timeToString(val))).optional(),
    max: z.literal("").transform(() => undefined).or(z.any().transform((val) => timeToString(val))).optional(),
    defaultValue: z.literal("").transform(() => undefined).or(z.any().transform((val) => timeToString(val))).optional()
});
...
export const schemaEffect = (schema: z.ZodObject<any, any>, editId: string | undefined, entity: ChangedFullEntity): z.ZodEffects<any, any> => {
    return schema.refine((data) => {
        ...
        .superRefine((data, ctx) => {
            ...
            if (data.min !== undefined && stringToTime(data.defaultValue)!.isBefore(stringToTime(data.min))) {
                ctx.addIssue({
            ...
            if (data.max !== undefined && stringToTime(data.defaultValue)!.isAfter(stringToTime(data.max))) {
                ctx.addIssue({
                    code: z.ZodIssueCode.custom,
                    path: ["defaultValue"],
...
export const toDefaultValue = (value: any): any => {
    ...
    return {
        ...value,
        min: value.min ? stringToTime(value.min) : value.min,
        max: value.max ? stringToTime(value.max) : value.max,
        defaultValue: value.defaultValue ? stringToTime(value.defaultValue) : value.defaultValue
    }
}
...
export const DesignerForm = (props: DesignerFormProps) => {
    ...
    return (<Stack gap={1.5}>
        ...
        <Stack direction="row" gap={1}>
            <TimeFormField name="min" label="Min Value"
                max={props.formMethods.watch('max')} />
            <TimeFormField name="max" label="Max Value"
                min={props.formMethods.watch('min')} />
        </Stack>

        <TimeFormField name="defaultValue" label="Default Value"
